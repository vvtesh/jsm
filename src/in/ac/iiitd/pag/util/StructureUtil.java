...
public class StructureUtil {
	...
	public static List<String> getAlgo(String codeFragment, boolean full) {
		if (!full) return getAlgo(codeFragment);
		final String code = codeFragment;
	...
	public static List<String> getComments(String codeFragment) {
		...
		node.accept(new ASTVisitor() {
			...
			public boolean visit(CompilationUnit node) {
				...
				for(Comment comment: comments) {
					...
					while (end < last) {
						if (code.charAt(end) == '\r' || code.charAt(end) == '\n') {
							break;
						}
						end++;
					}
					String commentinLine = code.substring(start, end);
					elements.add(commentinLine);
				}
				return super.visit(node);
			}
	...
	public static String stripComments(String codeFragment) {
		ASTNode node = ASTUtil.getASTNode(codeFragment);
